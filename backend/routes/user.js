...
router.get('/', async (req, res, next) => {
  try {
    ...
    const user = await User.find()
      .sort({ _id: -1 })
      .select({ _id: 1, username: 1, email: 1, followers: 1, followings: 1, tweets: 1 })
...
router.get('/feed', async (req, res, next) => {
  try {
    ...
    const tweets = await Tweet.find({
      ...
      .select({ message: 1, media: 1, mediaType: 1, likes: 1, retweets: 1, hashtags: 1, sender: 1, isRetweeted: 1, retweetedFrom: 1 })
      .populate({
        path: 'sender',
        model: User,
      })
...
router.get('/me', async (req, res, next) => {
  try {
    ...
    const tweets = await Tweet.find({
      sender: userId
    }).select({ message: 1, media: 1, mediaType: 1, likes: 1, retweets: 1, hashtags: 1, isRetweeted: 1, retweetedFrom: 1, sender: 1 }).sort({ _id: -1 }).populate({
      path: 'sender',
      model: User,
    })
